...
Refactored from: scripts/ai_agents.py
"""
...
from .config import calculate_cost, PRICING
...
class BaseAgent:
    ...
    def _calculate_cost(self, input_tokens: int, output_tokens: int) -> float:
        """Calculate cost based on model pricing using centralized PRICING."""
        return calculate_cost(self.model, input_tokens, output_tokens)

    def _call_api(
        self, messages: list, response_format: Optional[dict] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if content is None:
                logger.error(
                    f"API returned None content for {self.model}. "
                    f"Finish reason: {response.choices[0].finish_reason}"
                )
                logger.error(f"Full response: {response}")
            else:
                logger.debug(f"API returned content length: {len(content)}")
                if len(content) < 100:
                    logger.debug(f"Content snippet: {content}")
            ...
            if response.usage:
                ...
                self._last_cost = self._calculate_cost(
...
class WriterAgent(BaseAgent):
    ...
    def generate(
        self, product_data: Dict[str, Any], golden_example: Optional[Dict] = None
    ) -> Optional[str]:
        """
        ...
        Args:
            ...
            golden_example: Optional dictionary with 'input' (user prompt)
                          and 'output' (ideal response) for few-shot.
        """
